...
| M6' | [#243](https://github.com/kpubdata-lab/younggeul/issues/243) | `--render abdp\|legacy` CLI flag using `abdp.reporting.render_json_report` over the existing `RenderedReport` payload. **Now the next shippable milestone** — reporting is the first surface where semantics actually line up. |
| M7' | [#240](https://github.com/kpubdata-lab/younggeul/issues/240) | Simulation fit-gap matrix + runner-only adapters (no schema replacement, no AuditLog/Seed/snapshot dependency). |
...
| M9' | [#244](https://github.com/kpubdata-lab/younggeul/issues/244) | Shadow-mode `ScenarioRunner` **and** the real `AuditLog` adapter absorbed from #242. LangGraph stays the default execution engine. |
